...
public partial class LearnerPage : ContentPage
{
    ...
    private void Learner()
    {
        ...
        static (List<IOCard> inputCards, List<LogicGateCard> logicGateCards, List<OutputCard> outputCards)
            EfficientCalculateLogicState(LogicState state, List<bool> inputs)
        {
        ...
        void SearchForValidStateHeuristic()
        {
            ...
            Dispatcher.Dispatch(() => { DisplayAlert("Failure", "No valid state found.", "OK"); });
        }
        ...
        IEnumerable<LogicState> GenerateNeighborStates(LogicState currentState)
        {
            ...
            for (int sourceIndex = 0; sourceIndex < currentState.Gates.Count; sourceIndex++)
            {
                for (int targetIndex = 0; targetIndex < currentState.Gates.Count; targetIndex++)
                {
                    ...
                    if ((targetGate.StartsWith("Output") || targetGate.StartsWith("Not")) &&
                        currentState.Connections.Count(c => c.to == targetIndex) >= 1)
                        continue;

                    if (!targetGate.StartsWith("Input") && !targetGate.StartsWith("Output") &&
                        currentState.Connections.Count(c => c.to == targetIndex) >= 2)
                        continue;
